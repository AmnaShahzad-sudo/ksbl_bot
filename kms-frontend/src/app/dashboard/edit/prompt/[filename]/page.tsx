...
export default function EditPromptPage() {
  ...
  useEffect(() => {
    const fetchContent = async () => {
      ...
      try {
        const response = await axios.get(`${API_BASE}/admin/prompts/${filename}/content`, {
  ...
  const handleSave = async () => {
    ...
    const key = localStorage.getItem("ksbl_api_key");
    try {
      await axios.put(`${API_BASE}/admin/prompts/${filename}/content`, {
        content: content
      }, {
        headers: { "X-API-KEY": key }
      });
    ...
    <div className="flex flex-col h-screen bg-slate-50">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <div className="flex items-center gap-2">
              <MessageSquare className="w-4 h-4 text-[#002554]" />
      ...
      <div className="flex-1 p-8 overflow-hidden flex flex-col gap-4">
        {error ? (
          ...
          <div className="flex-1 bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col">
            <div className="bg-slate-50 px-4 py-2 border-b border-slate-200 flex items-center justify-between">
              <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">Configuration Text</span>
            ...
            <textarea
              ...
              spellCheck={false}
              placeholder="Configure the system prompt here..."
            />
          </div>
        )}
        ...
        <div className="flex items-center gap-2 text-[#002554] bg-[#002554]/5 p-3 rounded-xl border border-[#002554]/10">
          ...
          <p className="text-xs font-semibold text-[#002554]">Changes to system prompts take effect immediately for new chat sessions.</p>
        </div>
      </div>
    </div>
  );
